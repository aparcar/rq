...
from rq.suspension import (
    suspend as connection_suspend,
    resume as connection_resume,
    is_suspended,
)
...
shared_options = [
    ...
    click.option(
        "--config", "-c", envvar="RQ_CONFIG", help="Module containing RQ settings."
    ),
    click.option(
        "--worker-class",
        "-w",
    ...
    click.option(
        "--path", "-P", default=".", help="Specify the import path.", multiple=True
    ),
]
    ...
    if all:
        ...
        queues = [
            cli_config.queue_class(
                queue, connection=cli_config.connection, job_class=cli_config.job_class
            )
            for queue in queues
        ]
...
@click.option(
    "--raw", "-r", is_flag=True, help="Print only the raw numbers, no bar charts"
)
...
@pass_cli_config
def info(
    cli_config, interval, raw, only_queues, only_workers, by_queue, queues, **options
):
...
@click.option(
    ...
    help="Set the format of the logs",
)
@click.option(
    "--date-format",
    type=str,
    default=DEFAULT_LOGGING_DATE_FORMAT,
    help="Set the date format of the logs",
)
...
@click.option(
    "--disable-job-desc-logging", is_flag=True, help="Turn off description logging."
)
@click.option("--verbose", "-v", is_flag=True, help="Show more output")
@click.option("--quiet", "-q", is_flag=True, help="Show less output")
@click.option(
    "--sentry-ca-certs",
    envvar="RQ_SENTRY_CA_CERTS",
    help="Path to CRT file for Sentry DSN",
)
@click.option("--sentry-debug", envvar="RQ_SENTRY_DEBUG", help="Enable debug")
@click.option(
    "--sentry-dsn", envvar="RQ_SENTRY_DSN", help="Report exceptions to this Sentry DSN"
)
@click.option("--exception-handler", help="Exception handler(s) to use", multiple=True)
@click.option(
    "--pid", help="Write the process ID number to a file at the specified path"
)
@click.option(
...
@click.option(
    "--max-jobs", type=int, default=None, help="Maximum number of jobs to execute"
)
...
@click.option(
    "--duration",
    help="Seconds you want the workers to be suspended.  Default is forever.",
    type=int,
)
@pass_cli_config
