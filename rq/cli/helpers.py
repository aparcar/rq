...
def get_redis_from_config(settings, connection_class=Redis):
    ...
    if settings.get("REDIS_URL") is not None:
        ...
        sn = Sentinel(
            instances, socket_timeout=socket_timeout, password=password, db=db
        )
...
def show_workers(queues, raw, by_queue, queue_class, worker_class):
    ...
    if not by_queue:
        ...
        for worker in workers:
            ...
            if not raw:
                click.echo(
                    "%s: %s %s" % (name, state_symbol(worker.get_state()), queue_names)
                )
            else:
    ...
    def connection(self):
        if self._connection is None:
            if self.url:
                ...
                self._connection = get_redis_from_config(
                    settings, self.connection_class
                )
